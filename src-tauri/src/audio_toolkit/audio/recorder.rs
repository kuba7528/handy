...
enum Cmd {
    Start,
    /// Start continuous listening: the consumer keeps recording indefinitely
    /// and emits each detected utterance (speech followed by silence) through
    /// the segment callback instead of waiting for an explicit `Stop`.
    StartContinuous,
...
impl AudioRecorder {
    ...
    /// Register a callback invoked with the samples of each detected utterance
    /// while the recorder is in continuous listening mode.
    ...
    /// Begin continuous listening. Utterances are delivered through the segment
    /// callback until `stop`/`close` is called.
...
fn run_consumer(
    ...
    /// Process a single resampled frame. Returns `true` when the frame was
    /// classified as speech (and therefore appended to `out_buf`).
    fn handle_frame(
        samples: &[f32],
        recording: bool,
        vad: &Option<Arc<Mutex<Box<dyn vad::VoiceActivityDetector>>>>,
    ...
    loop {
        ...
        frame_resampler.push(&raw, &mut |frame: &[f32]| {
            let was_speech = handle_frame(frame, recording, &vad, &mut processed_samples);

            // Continuous mode: split the stream into utterances. Once speech has
            // been captured and is followed by a sustained run of silence, flush
            // the accumulated samples through the segment callback and keep
                ...
                if was_speech {
                    ...
                    if silence_run >= silence_frames {
                        if processed_samples.len() >= min_segment_samples {
                            ...
                            // Too short to be a real utterance; drop it.
