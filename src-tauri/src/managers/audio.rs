...
fn create_audio_recorder(
    ...
    let recorder = AudioRecorder::new()
        ...
        // In continuous listening mode each detected utterance is transcribed
        // and pasted automatically through this callback.
        .with_segment_callback({
            let app_handle = app_handle.clone();
            move |samples| {
                crate::actions::handle_continuous_segment(&app_handle, samples);
            }
        });
...
/// listening owns the microphone.
pub const CONTINUOUS_BINDING_ID: &str = "__continuous__";
...
impl AudioRecordingManager {
    ...
    /// Returns whether continuous listening is currently active.
    ...
    pub fn start_continuous_listening(&self) -> Result<(), String> {
        ...
        if *self.continuous.lock().unwrap() {
            debug!("Continuous listening already active");
            return Ok(());
        }

        if !matches!(*state, RecordingState::Idle) {
            return Err("Cannot start continuous listening while busy".to_string());
        }

        // Cancel any pending lazy close and ensure the stream is open.
        self.close_generation.fetch_add(1, Ordering::SeqCst);
        if let Err(e) = self.start_microphone_stream() {
            let msg = format!("{e}");
            error!("Failed to open microphone stream for continuous listening: {msg}");
            return Err(msg);
        }

        if let Some(rec) = self.recorder.lock().unwrap().as_ref() {
            if let Err(e) = rec.start_continuous() {
                return Err(format!("Failed to start continuous recording: {e}"));
            }
            *self.is_recording.lock().unwrap() = true;
